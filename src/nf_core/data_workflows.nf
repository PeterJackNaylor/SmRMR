...
include { simulate_data_unique; simulate_data_unique as sim_validation_unique} from './data_simulation.nf'
...
workflow simulation_train_validation_unique {
    take:
        simulation_models
        train_pairs_np
        validation_pairs_np
        repeat
    main:
        simulate_data_unique(simulation_models, train_pairs_np, 1..repeat,  "")
        prep_channel_init(simulate_data_unique.out).set{ train_split }

        sim_validation_unique(simulation_models, validation_pairs_np, 1, "_val")
        prep_channel_others(sim_validation_unique.out).set{ val_split }
...
workflow simulation_train_validation_test_unique {
    take:
        simulation_models
        train_pairs_np
        validation_pairs_np
    ...
    main:
        ...
        simulation_train_validation_unique(simulation_models, train_pairs_np, validation_pairs_np, repeat)

        sim_validation_unique(simulation_models, test_pairs_np, 1, "_test") .set{ sim_test }
        prep_channel_others(sim_test).set{ test_split }

        simulation_train_validation_unique.out .combine(test_split, by: 0) .set{ data }
    emit:
        data
}
